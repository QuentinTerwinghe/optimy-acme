...
use App\Mail\Payment\PaymentFailureMail;
...
/**
 * Handler for payment failure notifications.
 * Sends emails to payment initiators when a payment fails.
 */
class PaymentFailureHandler extends AbstractNotificationHandler
{
    /**
    ...
    {
        return NotificationType::PAYMENT_FAILURE;
    }
    ...
    /**
     * Send the payment failure notification.
     *
     * @param User $receiver
     * @param array<string, mixed> $parameters Expected keys:
     *                                         - payment: Payment (required) - The failed payment
     * @return bool
     */
    protected function send(User $receiver, array $parameters): bool
    {
        try {
            ...
            Log::info('Payment failure notification being sent', [
                'payment_id' => $payment->id,
                'receiver' => $receiver->email,
            ]);

            Mail::to($receiver->email)->send(
                new PaymentFailureMail(
                    receiver: $receiver,
