...
use App\Mail\Campaign\CampaignGoalAchievedMail;
...
/**
 * Handler for campaign goal achieved notifications.
 * Sends emails to campaign creators when their campaign goal is achieved.
 */
class CampaignGoalAchievedHandler extends AbstractNotificationHandler
{
    /**
    ...
    {
        return NotificationType::CAMPAIGN_GOAL_ACHIEVED;
    }
    ...
    /**
     * Send the campaign goal achieved notification.
     *
     * @param User $receiver
     * @param array<string, mixed> $parameters Expected keys:
     *                                         - campaign: Campaign (required) - The campaign that achieved its goal
     * @return bool
     */
    protected function send(User $receiver, array $parameters): bool
    {
        try {
            ...
            Log::info('Campaign goal achieved notification being sent', [
                'campaign_id' => $campaign->id,
                'receiver' => $receiver->email,
            ]);

            Mail::to($receiver->email)->send(
                new CampaignGoalAchievedMail(
                    receiver: $receiver,
                    campaign: $campaign->toArray(),
                )
            );

            Log::info('Campaign goal achieved notification sent');
