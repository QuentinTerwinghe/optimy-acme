...
final class CampaignMapper
{
    ...
    /**
     * Convert UpdateCampaignRequest to UpdateCampaignDTO
     *
     * Only includes fields that are present in the request (partial updates)
     */
    public static function fromUpdateRequest(UpdateCampaignRequest $request): UpdateCampaignDTO
    {
        $validated = $request->validated();
        ...
        return new UpdateCampaignDTO(
            ...
            startDate: isset($validated['start_date']) ? Carbon::parse($validated['start_date']) : null,
            endDate: isset($validated['end_date']) ? Carbon::parse($validated['end_date']) : null,
            status: isset($validated['status']) ? CampaignStatus::fromRequest($validated['status']) : null,
            description: $validated['description'] ?? null,
            categoryId: $validated['category_id'] ?? null,
            currentAmount: isset($validated['current_amount']) ? (float) $validated['current_amount'] : null,
            tags: $validated['tags'] ?? null
